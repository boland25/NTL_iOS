...
void SqrRootMod(ZZ& x, const ZZ& aa, const ZZ& nn)
{
   ...
   n = nn;
   nlen = static_cast<int>(NumBits(n));

   sub(m, n, 1);
   h = static_cast<int>(MakeOdd(m));  // h >= 2
            ...
            long hlen=SHA256_HASHSIZE)
{
   ...
   if (i < dlen)
      sha256_update(ctx, data + i, static_cast<_ntl_uint32>(dlen - i));
                 ...
                 unsigned char *hash, long hlen=SHA256_HASHSIZE)
{
   ...
   sha256_update(ctx, data, static_cast<_ntl_uint32>(dlen));
